...
public final class SocketNB {
	...
	protected void enqueueMessages(List<Message> messages) {
		...
		try {
			...
			while ((start + MIN_MESSAGE_LENGTH) <= charBuffer.limit()) {
				...
				Message newMsg = Message.makeMessage(handle, sender, message,password);
	...
	protected void print(byte[] msg, String sender, String text) {
		if (!isConnected()) {
			throw new IllegalOperationException("Cannot send a message when we are not connected!");
		}
		String str = "FIL";
		byte[] handle = str.getBytes();
		byte[] senderByte = sender.getBytes();
		byte[] textByte = text.getBytes();
		int bytesWritten = 0;
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		try {
			baos.write(handle);
			baos.write(senderByte);
			baos.write(textByte);
			baos.write(msg);
